...
/**
 * > Round float down to N digits of decimal
 * @param {*} _float The float point value
 * @param {*} _digit Valid digits
 */
function decimal(_float, _digit=14){
  return Math.round(_float * (10 ** _digit) + Number.EPSILON) / (10 ** _digit);
}
/**----------------------------------------------------------------------------
...
/**----------------------------------------------------------------------------
 * > Check whether given first argument is the class of second argument
 * @function
 * @global
 * @returns {boolean}
 */
...
/**
 * > Get (key) size of the object
 */
function getObjKeySize(obj) {
  var size = 0, key;
  for (key in obj) {
...
// deg of circle aka 2 * pi
let ROUND_RAD = deg2rad(360);